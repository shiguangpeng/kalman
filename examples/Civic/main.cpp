...
int main(int argc, char *argv[]) {
  ...
  Vector3f U(3);
  U << (1 / ALPHA(m_freq))
	  * (-1 * T(gap_t) + 0.5 * (A_T(m_freq, gap_t * gap_t)) + (1 - EXP(-1 * A_T(m_freq, gap_t))) / (ALPHA(m_freq))),
	  T(gap_t) - (1 - EXP(-1 * A_T(m_freq, gap_t))) / (ALPHA(m_freq)),
	  1 - EXP(-1 * A_T(m_freq, gap_t));

  Matrix3f Q(3, 3);
  float Q_11 = (0.5 * INDEX(ALPHA(m_freq), -5))
	  * (1 - EXP(-2 * A_T(m_freq, gap_t)) + (2 * A_T(m_freq, gap_t)) + (2 / 3 * (INDEX(A_T(m_freq, gap_t), 3)))
		  - (2 * INDEX(A_T(m_freq, gap_t), 2)) - (4 * A_T(m_freq, gap_t) * EXP(-1 * A_T(m_freq, gap_t))));
  float Q_12 = (0.5 * INDEX(ALPHA(m_freq), -4)) * (EXP(-2 * A_T(m_freq, gap_t)) + 1 - 2 * EXP(-1 * A_T(m_freq, gap_t))
	  + 2 * A_T(m_freq, gap_t) * EXP(-1 * A_T(m_freq, gap_t)) - 2 * A_T(m_freq, gap_t) + INDEX(A_T(m_freq, gap_t), 2));
  float Q_13 = (0.5 * INDEX(ALPHA(m_freq), -3))
	  * (1 - EXP(-2 * A_T(m_freq, gap_t)) - 2 * A_T(m_freq, gap_t) * EXP(-1 * A_T(m_freq, gap_t)));
  float Q_22 = (0.5 * INDEX(ALPHA(m_freq), -3))
	  * (4 * EXP(-1 * A_T(m_freq, gap_t)) - 3 - EXP(-2 * A_T(m_freq, gap_t)) + 2 * A_T(m_freq, gap_t));
